import { clamp, lerp, unclamp, unlerp } from "./math";
...
export default class Vec2 {
    ...
    /**
     * Creates a Vec2 where x: n and y: n
     */
    constructor(n: number);

    /**
     * @param v object containing x and y properties
     */
    constructor(v: { x?: number, y?: number });

    /**
     * @param x if null, defaults to 0
     * @param y if null, defaults to value of x, which defaults to 0
     */
    ...
    public unclamp(min: Vec2, max: Vec2) {
        return new Vec2(unclamp(this.x, min.x, max.x), unclamp(this.y, min.y, max.y));
    }
    ...
    /**
     * new Vec2(num ?? 0, num2 ?? num ?? 0)
     */
    static From = (num: number, num2?: number) => new Vec2(num, num2);

    /**
     * @returns a shallow copy of this Vec2
     */
    clone(): Vec2 {
        return new Vec2(this.x, this.y);
    }
